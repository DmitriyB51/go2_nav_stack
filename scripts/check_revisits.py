#!/usr/bin/env python3
"""Per revisit candidate: build a local submap at BOTH keyframes and ICP them
against each other from the pose the map already claims. How far ICP has to move
it = the real remaining error at that place."""
...
W = 1.0          # [s] half-window of scans per submap
...
def raw_pose(t):
    """Nearest raw Point-LIO pose — locally (~2 s) accurate."""
    i = int(np.argmin(np.abs(odo[:,0]-t))); r = odo[i]
    return T(q2R(*r[4:8]), r[1:4])

def submap(kf):
    """Scans within +-W s, in the keyframe's frame."""
...
def icp(src, dst, T0, iters=40, max_corr=1.0, trim=0.8, rot=True):
    """Trimmed point-to-point ICP -> correction on top of T0."""
    ...
    for _ in range(iters):
        ...
        if ok.sum() < 30:
            break
        thr = np.quantile(d[ok], trim)           # drop the worst matches
        ok &= d <= thr
        A, B = s[ok], dst[idx[ok]]
...
def rms_to(src, dst, Tx, max_corr=1.0):
    """How far apart the two passes are under a given transform."""
    tree = cKDTree(dst)
...
for i, j in EVENTS:
    ...
    T0 = np.linalg.inv(kf_T[i]) @ kf_T[j]      # what the map claims
    rms0, med0, ovl = rms_to(Sj, Si, T0)

    # translation-only: robust in corridors, no rotation to slide into
