/* plio2sm: Point-LIO output rosbag2 -> MOLA/MRPT .simplemap.
 *
 * In:  odometry (nav_msgs/Odometry) = per-scan T_world_from_body
 *      cloud (sensor_msgs/PointCloud2) = body-frame scan, already deskewed
 * Out: keyframes of (pose PDF, CObservationPointCloud), cloud in the sensor
 *      frame with sensorPose=Identity -> keyframe placement = odometry pose.
 *      Same structure MOLA-LO builds; only the pose source differs.
 */
...
int main(int argc, char** argv)
{
  ...
  // Anchor every kfDist of path length; each keyframe accumulates all scans
  // within +/- submapRadius (same pass -> consistent poses). Dense overlapping
  // clouds are far more robust for loop-closure ICP than single scans.
